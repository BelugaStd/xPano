        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          {step === 1 && (
            <div>
              ...
              {config.backend === 'metashape' ? <div className="mt-5 grid grid-cols-2 gap-4"><label className="text-[11px] text-muted"><span className="mb-1.5 block font-medium text-ink/75">对齐策略</span><select value={config.alignmentMode} onChange={(event) => onChange({ ...config, alignmentMode: event.target.value as ReconstructionConfigDraft['alignmentMode'] })} className="theme-input h-10 w-full rounded-comfortable border px-3 text-[12px]"><option value="backbone">骨架模式（推荐）</option><option value="mixed">混合模式（高级）</option></select></label><label className="text-[11px] text-muted"><span className="mb-1.5 block font-medium text-ink/75">关键点上限</span><input type="number" value={config.metashapeKeypointLimit} onChange={(event) => onChange({ ...config, metashapeKeypointLimit: Math.max(0, Number(event.target.value) || 0) })} className="theme-input h-10 w-full rounded-comfortable border px-3 text-[12px]" /></label><div className="col-span-2 rounded-comfortable border border-[var(--xp-line)] p-3 text-[10px] leading-5 text-muted">当前工程包含 {tracks.length} 条素材轨、{tracks.reduce((sum, track) => sum + track.items.filter((item) => item.selected).length, 0)} 个已选素材项。{hasFlat ? '检测到平面素材，推荐骨架模式。' : '当前为纯全景素材。'}</div></div> : <div className="mt-5 grid grid-cols-2 gap-4"><label className="text-[11px] text-muted"><span className="mb-1.5 block font-medium text-ink/75">匹配策略</span><select value={config.colmapMatcher} onChange={(event) => onChange({ ...config, colmapMatcher: event.target.value as ReconstructionConfigDraft['colmapMatcher'] })} className="theme-input h-10 w-full rounded-comfortable border px-3 text-[12px]"><option value="sequential">顺序匹配</option><option value="exhaustive">穷举匹配</option></select></label><label className="text-[11px] text-muted"><span className="mb-1.5 block font-medium text-ink/75">GPU</span><span className="flex h-10 items-center justify-between rounded-comfortable border border-[var(--xp-line)] px-3 text-ink/75">CUDA 加速<input type="checkbox" checked={config.colmapUseGpu} onChange={(event) => onChange({ ...config, colmapUseGpu: event.target.checked })} className="h-4 w-4 accent-[var(--xp-brand)]" /></span></label></div>}
            </div>
          )}
